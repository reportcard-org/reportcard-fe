...
describe('User Favorites page', () => {
    ...
    it('can search for a district if user inputs a city, street, and state', () => {
        cy.get('.search-form-container').should('exist')
            ...
            .get('p').eq(3).contains(18.07)
            .get('p').eq(4).contains('Teacher salary: ')
            .get('p').eq(5).contains('$73,400.51')
            .get('p').eq(6).contains('Instruction Salary % of Total: ')
    ...
    it('should allow users to return to the search page and look for a different address', () => {
        cy.get('.fav-search-page').click()
           .url().should('eq', 'http://localhost:3000/home')
            ...
            .get('.title-and-info-container')
            .get('p').first().contains('Back to Search')
            .get('p').eq(3).contains(14.95)
            .get('p').eq(4).contains('Teacher salary: ')
            .get('p').eq(5).contains('$65,475.06')
            .get('p').eq(6).contains('Instruction Salary % of Total: ')
            .get('p').eq(7).contains(50.40)
            .get('p').eq(8).contains('Enrollment: ')
            .get('p').eq(9).contains('92,039')
            .get('p').eq(10).contains('Number of Schools: ')
            .get('p').eq(11).contains(206)
            .get('p').eq(12).contains('Guidance Counselor Ratio: ')
            .get('p').eq(13).contains(484.42)
            .get('p').eq(14).contains('Expenditure Per Student: ')
            .get('p').eq(15).contains('$16,752.85')
    })
